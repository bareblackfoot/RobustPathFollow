...
def quat2rot(quats):
    if isinstance(quats,np.ndarray):
        quats = q.as_quat_array(quats)
    ret = q.as_rotation_vector(quats)
    ret = ret.reshape(-1,3)
    ret = ret[:,1]-3.14
...
class DemoRunner:
    ...
    def save_color_observation(self, obs, total_frames):
        ...
        color_img.save("./scene/test.rgba.%05d.png" % total_frames)
    ...
    def step(self, in_action):
        ...
        self.dist_to_goal = np.linalg.norm(self._sim.agents[0].get_state().position[:2] - self.end_position[:2])
        self.rot_to_goal = np.linalg.norm(quat2rot(self._sim.agents[0].get_state().rotation) - quat2rot(self.end_rotation))
    ...
    def init_common(self):
        ...
        self.initial_dist_to_goal = np.linalg.norm(start_state.position[:2] - self.end_position[:2])
        self.dist_to_goal = np.linalg.norm(start_state.position[:2] - self.end_position[:2])
